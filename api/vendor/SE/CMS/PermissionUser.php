...
class PermissionUser extends Base
{
    ...
    public function fetch($isId = false)
    {
        try {
            $u = new DB('se_user', 'su');
            $u->select('su.id, su.created_at reg_date, su.person_name, 
            ...
            $u->leftJoin('se_permission_role_user pru', 'pru.id_user = su.id');
            $u->leftJoin('se_permission_role pr', 'pr.id = pru.id_role');
            ...
            foreach ($result as $item) {
                ...
                $manager['personName'] = $item['personName'];
                $manager['username'] = $item['username'];
                $manager['title'] = $item['personName'];
        ...
        try {
            if ($ids) {
                if (!empty($this->input["idsRoles"])) {
                    ...
                    $u = new DB("se_permission_role_user", "pru");
                    $u->where("id_user IN (?)", $idsStr);
                    $u->andWhere("NOT id_role IN (?)", $idsRoles);
                    $u->deleteList();
                    foreach ($ids as $id)
                        foreach ($this->input["idsRoles"] as $idRole) {
                            $sql = "INSERT IGNORE INTO se_permission_role_user (id_user, id_role) VALUE ({$id}, {$idRole})";
                            DB::exec($sql);
                        }
